...
export default function BountyQuestionCard({
  question,
  onSolve,
  questionIndex,
}: BountyQuestionCardProps) {
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case "R2_BOUNTY":
        return "text-purple-600"; // Example for bounty difficulty
      default:
    ...
    <>
      ...
      <div
        className={`w-32 h-20 rounded-lg shadow-lg mt-2 ml-2 p-2 cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 ${
          isBountyClaimed ? "bg-cyan-700" : "bg-orange-700" // Color reflects if bounty is claimed
          ...
          >
            ...
            <div className="space-y-4 oxanium">
              <div className="flex items-center gap-3 flex-wrap">
                <span
                  className={`inline-block px-3 py-1 rounded-full text-sm font-medium border ${getDifficultyBg(question.difficulty)} ${getDifficultyColor(question.difficulty)}`}
                >
                ...
                >
                  <CheckCircle
                    className={`w-4 h-4 ${hasUserSolved ? "text-green-600" : "text-red-600"}`}
                  />
