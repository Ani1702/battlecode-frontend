...
export default function CodePage({ round }: CodePageProps) {
  ...
  const saveSession = useCallback(() => {
    if (questionSession && currentQuestion) {
      ...
      localStorage.setItem(
        `question_session_${currentQuestion.id}`,
  ...
  const getTimerDisplay = (): { time: string; className: string } => {
    ...
    const isWarning = QuestionManager.isTimeWarning(
      timeLeft,
      currentQuestion.timeLimit,
    );
    const isCritical = QuestionManager.isTimeCritical(
      timeLeft,
      currentQuestion.timeLimit,
    );
  ...
  function handleSubmit() {
    ...
    setTimeout(() => {
      ...
      const allPassed = mockResults.every(
  ...
  return (
    <div className="flex flex-col h-screen  text-white overflow-hidden bg-amber-800">
      ...
      <div className="flex-1 flex p-4 gap-4 bg-black/40 backdrop-blur-sm min-h-0">
          ...
          <div className="flex justify-between items-start mb-4 flex-shrink-0">
            <div>
              ...
              <div className="flex gap-4 text-sm text-gray-400 mt-1">
                <span
                  className={`px-2 py-1 rounded ${QuestionManager.getDifficultyColor(currentQuestion!.difficulty)}`}
                >
                  {currentQuestion!.difficulty}
                </span>
                <span>Points: {currentQuestion!.points}</span>
                <span>
                  Time: {Math.floor(currentQuestion!.timeLimit / 60)}min
                </span>
            ...
            {currentQuestion!.sampleTestCases.map((testCase, i) => (
              ...
              >
                ...
                <pre className="bg-gray-900 p-2 rounded mt-1 whitespace-pre-wrap">
                  {formatTestCaseData(
                    testCase.input.stdin || testCase.input.json,
                  )}
                </pre>
                <p className="mt-2 font-bold text-gray-400">Output:</p>
                <pre className="bg-gray-900 p-2 rounded mt-1 whitespace-pre-wrap">
                  {formatTestCaseData(
                    testCase.output.stdout || testCase.output.json,
                  )}
                </pre>
        ...
        {/* Code & Results Panel */}
        <div
          className="w-1/2 flex flex-col code-results-container"
          style={{ height: "100%" }}
        >
          {/* Code Editor */}
          <div
          ...
          >
            <div className="flex justify-between items-center mb-2 gap-50">
              <div className="flex-1 flex ">
                ...
                <div
                  className={`bg-gray-800 flex-[0.7] text-white p-2 rounded border focus:outline-none focus:ring-2 focus:ring-amber-500 ml-1 text-center font-mono ${
