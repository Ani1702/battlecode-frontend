    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        Object.keys(localStorage).forEach(key => {
          if (key.startsWith('sb-') || key.startsWith('supabase') || key.includes('auth') || key.includes('stuck_session')) {
      ...
      <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
        <div className="text-center">
          ...
          <div className="mt-6 space-y-3">
            ...
            >
              {isClearing ? 'Clearing Session...' : 'Clear Session & Return Home'}
            </button>
            <button
              onClick={() => router.push('/')}
          ...
          {reason === 'backend_connection_failed' && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-800">
                The backend server appears to be down. Please contact support or try again later.
              </p>
            </div>
          )}
        </div>
