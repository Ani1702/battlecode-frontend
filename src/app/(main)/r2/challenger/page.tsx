...
export default function ChallengerDashboard() {
  ...
  // Guard: Redirect if user has active session or is not a challenger
  useEffect(() => {
    if (!state || stateLoading) return;

    console.debug('[Challenger] State check:', {
    ...
    // If user is not a challenger, redirect to their role page
    if (state.state?.currentUser.role !== 'challenger') {
  ...
  useEffect(() => {
    if (!socket || !isConnected || !state || state.state?.currentUser.role !== 'challenger') return;
  ...
  if (stateLoading || dashboardLoading) {
    return <LoadingOverlay isLoading={true} message="Loading Challenger Dashboard..." />;
  }
